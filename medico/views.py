...
from paciente.models import Consulte, Documento
...
def consultas_medico(request):
    ...
    if request.method == 'GET':
        consultas_hoje = Consulte.objects.filter(data_aberta__user = request.user).filter(data_aberta__data__gte = hoje).filter(data_aberta__data__lt = hoje + timedelta(days=1))
        consultas_restantes = Consulte.objects.exclude(id__in=consultas_hoje.values('id')).filter(data_aberta__user = request.user)
...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == "GET":
        ...
        if request.user != consulta.data_aberta.user:
            messages.add_message(request, constants.ERROR, 'Essa consulta não pertece a este usuário!')
            redirect('/medicos/consultas_medico')
        ...
        if consulta.status == 'C':
            messages.add_message(request, constants.WARNING, 'Essa consulta já foi cancelada! Não é possível iniciar.')
            return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
        elif consulta.status == "F":
            messages.add_message(request, constants.WARNING, 'Essa consulta já foi finalizada! Não é possível iniciar.')
            return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
...
def finalizar_consulta(request, id_consulta):
    ...
    if consulta.status != 'I':
        messages.add_message(request, constants.WARNING, 'Essa consulta ainda não foi iniciada')
        redirect(f'/medicos/consulta_area_medico/{id_consulta}')
    consulta.status = 'F'
    consulta.save()
    return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
...
def dashboard(request):
    if not is_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Somente médicos podem acessar essa página.')
        return redirect('/usuarios/sair')
    consultas = Consulte.objects.filter(data_aberta__user = request.user).filter(data_aberta__data__range = [datetime.now().date() - timedelta(days=7), 
                datetime.now().date() + timedelta(days=1)]).values('data_aberta__data').annotate(quantidade = Count('id'))
